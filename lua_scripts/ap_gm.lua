...
function AP.HandleGMCommand(player, rawMsg)
    ...
    local function Reply(text)
        AP.Try(function()
            AP.RT.SendMessage(player,"|cffff9900[Worldsoul GM]|r " .. text)
        end, "GM reply")
    end
    ...
    if aetherAmt then
        ...
        if amount and amount > 0 then
            AP.GrantAether(AP.RT.GetGUID(player), amount)
        ...
        for _, slot in ipairs(slots) do
            AP.Try(function()
                ...
                if item then
                    ...
                    AP.SaveItemAttune(guid, AP.RT.GetItemEntry(item), AP.Config.CapPerItem, true)
                    count = count + 1
                end
