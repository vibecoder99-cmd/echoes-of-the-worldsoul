...
local function OnHonorableKill(event, killer, killed)
    ...
    pcall(function()
        local guid   = AP.RT.GetGUID(killer)
        local amount = AP.PvP.Values.honorKillBase
        GrantEssence(guid, amount)
        AP.RT.SendMessage(killer,string.format(
...
AP.RT.RegisterEvent("player", 6, OnHonorableKill)
...
-- We iterate AP.RT.GetPlayersInWorld() and filter by InBattleground()
...
local function OnBGEnd(event, bg, bgId, instanceId, winner)
    ...
    pcall(function()
        local allPlayers = AP.RT.GetPlayersInWorld()
        if not allPlayers then return end
        for _, player in ipairs(allPlayers) do
            pcall(function()
                if not AP.RT.InBattleground(player) then return end
                if AP.RT.GetBattlegroundId(player) ~= instanceId then return end
                local guid     = AP.RT.GetGUID(player)
                local isWinner = (AP.RT.GetTeam(player) == winner)
...
AP.RT.RegisterBGEvent(BG_EVENT_ON_END, OnBGEnd)
