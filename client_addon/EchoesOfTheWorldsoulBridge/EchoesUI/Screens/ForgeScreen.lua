...
function Screen:Begin(action,entry) if self.pending then return false end; self.pending=true; self.pendingAction=action; self.status:SetText(action=="forge_dissolve" and "Dissolution reserved. Do not retry while the terminus settles." or "Catalyst is settling…"); self:Refresh(UI.StateStore.values); APB:RequestEchoesAction(action,entry); self.token=(self.token or 0)+1; local token=self.token; C_Timer.After(7,function() if Screen.active and Screen.pending and Screen.token==token then Screen.pending=false; Screen.status:SetText("No response received. Verify state before trying again."); Screen:Refresh(UI.StateStore.values); APB:RequestEchoesState() end end); return true end
function Screen:Dissolve()
    ...
    if self.armedEntry~=self.selected then
        ...
        self.status:SetText("Dissolution armed. Activate again within 6 seconds to destroy the physical item.")
...
function Screen:OnAction(verb,fields) if not self.active or not self.pending then return end; self:Disarm(); if verb=="ACTION_OK" and fields.action==self.pendingAction then self.pending=false; self.token=(self.token or 0)+1; if fields.status=="SUCCESS" then self.status:SetText(fields.action=="forge_dissolve" and ("Relic released: +"..FormatInt(fields.essence_reward).." Essence, +"..FormatInt(fields.residue_reward).." Residue. Permanent Attunement remains.") or "Catalyst exchange complete."); self.selected=nil else self.status:SetText(fields.recoverable=="1" and "The item was removed; durable recovery will complete the reward on relog." or (fields.reason~="" and fields.reason or fields.status)) end; self:Refresh(UI.StateStore.values); APB:RequestEchoesState() elseif verb=="ERROR" then self.pending=false; self.token=(self.token or 0)+1; self.status:SetText("Forge request unavailable: "..tostring(fields.code)); self:Refresh(UI.StateStore.values) end end
