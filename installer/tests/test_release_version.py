...
def test_current_release_versions_are_reconciled():
    assert '## Version: 2.1.2' in (ROOT / 'client_addon/EchoesOfTheWorldsoulBridge/EchoesOfTheWorldsoulBridge.toc').read_text(encoding='utf-8')
    assert 'UI.version = "2.1.2"' in (ROOT / 'client_addon/EchoesOfTheWorldsoulBridge/EchoesUI/Bootstrap.lua').read_text(encoding='utf-8')
    assert 'AP.VERSION = "2.1.2"' in (ROOT / 'lua_scripts/ap_core.lua').read_text(encoding='utf-8')
    commands = (ROOT / 'lua_scripts/ap_commands.lua').read_text(encoding='utf-8')
    assert 'AP.CLIENT_ADDON_VERSION = "2.1.2"' in commands
